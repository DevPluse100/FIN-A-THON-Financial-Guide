...
const FamilyDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:8000/api/v1/users/famexp");
  ...
  const handleAddData = async (entry) => {
    try {
      ...
      const response = await axios.post("http://localhost:8000/api/v1/users/addfamexp", entry);
  ...
  return (
    <div className="dashboard">
      <h2 style={{marginLeft:"3rem",marginTop:"8rem"}}>Family Expenditure Dashboard</h2>
